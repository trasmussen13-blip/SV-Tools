...
.CATEGORY
    Tools
.SYNOPSIS
    Creates or resets a local service account and seats it as the logon account for AXM services.
.DESCRIPTION
    This PowerShell script creates or resets a local account (svcaxm_usr) with a random password,
    adds the account to the local Administrators group, grants the "Log On As A Service" right,
    and configures AXM-related services (axm*) to run under that account. The script:
      - Generates a 32-character random password and stores it only in memory.
      - Creates the local user or resets its password if the account exists.
      - Sets the account password to never expire.
      - Adds the account to the local Administrators group (looked up by SID to handle localization).
      - Grants SeServiceLogonRight via secedit export/modify/import.
      - Discovers AXM services and excludes VnHostSrv and CommNodeSrv from seating (they are still handled for restart).
      - Stops targeted services and their dependents, attempts to set the service StartName using CIM (Invoke-CimMethod Change),
        falls back to sc.exe if needed, and verifies the configured StartName.
      - Optionally assigns filesystem and registry ACLs (Modify/FullControl) for service binary directories and
        service registry keys.
      - Restarts services in dependency order and validates that services are Running and use the expected account.
    The script uses CIM and sc.exe fallbacks, and will attempt to handle dependent services when stopping/starting.
.ADMIN #admin rights needed
    YES
.EXAMPLE
    PowerShell (run as Administrator):
        .\AXM_SeatServiceUser.ps1
.NOTES
    - The script requires elevation. It is recommended to run with -RunAsAdministrator (the script includes a #Requires directive).
    - The generated password is only shown in-memory during execution and is not saved to disk by the script.
    - Uses secedit.exe, sc.exe, Get-LocalUser/New-LocalUser/Set-LocalUser, and CIM (Get-CimInstance/Invoke-CimMethod).
    - Intended for Windows PowerShell on systems that support the LocalAccounts module and secedit.
#>
